...
Iulita автоматически регистрирует Claude Haiku как дешёвый провайдер при наличии API-ключа Claude.

### Автоматическая маршрутизация фоновых задач

Фоновые задачи (сжатие контекста, генерация инсайтов, анализ профиля, уточнение закладок, heartbeat) автоматически направляются на Haiku через `RouteHint`.

### Маршрутизация на уровне скиллов

Скиллы могут объявить, что синтез ответа после их выполнения может использовать дешёвую модель через интерфейс `SynthesisModelDeclarer`. Скиллы с дешёвым синтезом: `datetime`, `exchange_rate`, `geolocation`, `recall`, `list_insights`, `websearch`.

### Маршрутизация суб-агентов

| Тип агента | Маршрут | Причина |
|------------|---------|---------|
| summarizer | claude-haiku | Суммаризация |
| researcher | Sonnet (по умолчанию) | Требуется рассуждение |
| analyst | Sonnet (по умолчанию) | Анализ паттернов |
