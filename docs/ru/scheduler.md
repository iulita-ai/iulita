...
### Уточнение закладок (`bookmark.refine`)

- **Триггер**: по запросу (создаётся `bookmark.Service.Save`)
- **Тип задачи**: `bookmark.refine`
- **Capabilities**: `llm,storage`
- **Максимум попыток**: 2
- **Удаление после выполнения**: да

**Обработчик**: Получает `{fact_id, content, chat_id, user_id}`. Вызывает LLM с промптом суммаризации для извлечения 1-3 кратких предложений. Обновляет содержимое факта, если уточнение существенно короче (<90% от оригинала). Корректно обрабатывает уже удалённые факты.

**Не является запланированной задачей** — задачи создаются по запросу, когда пользователи нажимают кнопку закладки. Воркер подхватывает их в следующем цикле опроса (каждые 5 секунд).
