...
| Метод | Путь | Описание |
|-------|------|----------|
| GET | `/api/usage/summary` | Общая сводка: токены, стоимость, попадания кэша |
| GET | `/api/usage/daily` | Ежедневная разбивка за последние 30 дней |
| GET | `/api/usage/by-model` | Использование в разрезе моделей |

### Страница дашборда

Страница Token Usage отображает:

- **KPI-карточки** — общее количество входных/выходных токенов, оценка стоимости, процент попаданий в кэш
- **Таблица по моделям** — использование токенов и стоимость для каждой модели (Claude Sonnet, Haiku, Ollama и т.д.)
- **Ежедневная разбивка** — график использования за последние 30 дней

### Скилл в чате

Скилл `token_stats` доступен через чат (только для администраторов). Позволяет запросить статистику использования токенов прямо в разговоре.
