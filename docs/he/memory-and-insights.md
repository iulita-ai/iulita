...
בנוסף לזרימת "תזכור" מבוססת צ'אט, משתמשים יכולים לסמן כל תשובת עוזר בלחיצה אחת.

### איך זה עובד

1. **Telegram**: כפתור 💾 במקלדת inline מופיע מתחת לכל תשובת עוזר (כולל הודעות רב-נתחיות)
2. **WebChat**: סמל 💾 מופיע בהעברת עכבר מעל הודעות העוזר
3. לחיצה על הכפתור שומרת **מיידית** את התשובה המלאה כעובדה עם `source_type="bookmark"`
4. משימת מתזמן ברקע (`bookmark.refine`) שולחת את התוכן ל-LLM לסיכום
5. אם ה-LLM מייצר גרסה קצרה משמעותית (<90% מהאורך המקורי), תוכן העובדה מתעדכן

### סימניה מול זכירה

| היבט | סימניה (כפתור 💾) | זכירה ("תזכור ש...") |
|------|---------------------|-------------------------------|
| טריגר | לחיצת כפתור | הודעת צ'אט |
| תוכן | תשובה מלאה של העוזר | עובדות מפתח שחולצו על ידי LLM |
| מהירות | מיידי (<5ms) | 2-5 שניות (קריאת LLM) |
| עלות טוקנים | מושהית (עידון ברקע) | מיידית |
| סוג מקור | `bookmark` | `user` |
| בדיקת כפילויות | אין (שומר כפי שהוא) | בדיקת FTS על 3 מילים ראשונות |

### עידון ברקע

משימת המתזמן `bookmark.refine`:
- **יכולות**: `llm,storage`
- **ניסיונות מקסימום**: 2
- **מחיקה לאחר ריצה**: כן (חד-פעמי)
- מחלץ 1-3 משפטים תמציתיים מהתשובה שנשמרה בסימניה
- מדלג על עדכון אם ה-LLM מחזיר ריק או שהעידון אינו קצר יותר
- מטפל בחינניות בעובדות שנמחקו (המשתמש עשוי למחוק לפני שהעידון רץ)

### אבטחה

- **Telegram**: שולח ה-callback מאומת מול מקבל ההודעה (בדיקת `tgUserID`)
- **WebChat**: מטמון ההודעות שומר בעלי `chatID`; הבעלות מאומתת לפני שמירה
