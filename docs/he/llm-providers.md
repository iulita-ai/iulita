...
Iulita רושמת אוטומטית את Claude Haiku כספק זול כאשר מוגדר מפתח API של Claude.

### ניתוב אוטומטי של משימות רקע

משימות רקע (דחיסת הקשר, יצירת תובנות, ניתוח פרופיל, עידון סימניות, heartbeat) מנותבות אוטומטית ל-Haiku.

### ניתוב ברמת כישורים

כישורים יכולים להצהיר שהסינתזה אחרי ביצועם יכולה להשתמש במודל זול יותר באמצעות `SynthesisModelDeclarer`. כישורים עם סינתזה זולה: `datetime`, `exchange_rate`, `geolocation`, `recall`, `list_insights`, `websearch`.

### ניתוב תת-סוכנים

| סוג סוכן | ניתוב | סיבה |
|----------|-------|------|
| summarizer | claude-haiku | סיכום |
| researcher | Sonnet (ברירת מחדל) | דורש חשיבה |
| analyst | Sonnet (ברירת מחדל) | זיהוי דפוסים |

### תמחור מודלים (למיליון טוקנים)
