...
| 方法 | 路径 | 描述 |
|------|------|------|
| GET | `/api/usage/summary` | 总览摘要：令牌数、费用、缓存命中率 |
| GET | `/api/usage/daily` | 最近30天每日明细 |
| GET | `/api/usage/by-model` | 按模型分类的使用量 |

### 仪表盘页面

Token Usage 页面展示：

- **KPI 卡片** — 输入/输出令牌总数、费用估算、缓存命中率
- **按模型分类表** — 每个模型（Claude Sonnet、Haiku、Ollama 等）的令牌使用量和费用
- **每日明细** — 最近30天的使用量图表

### 聊天技能

`token_stats` 技能可通过聊天使用（仅限管理员）。允许在对话中直接查询令牌使用统计。
